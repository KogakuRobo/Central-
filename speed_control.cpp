...
speed_control::speed_control(target_point*pt_posi,my_position*pmy_posi):pid_x(20, 0, 1, 0.01), pid_y(2, 0, 0, 0.01),pid_yaw(180, 0, 9, 0.01) {
	wheel =0;
	output = 0;
	output_B = 0;
	theta = 0;
...
double speed_control::output_F_get(int WHEEL) {
	...
	switch(wheel) {
		...
		//	output = -1*(- V[X]*sin(theta+pi/4)+V[Y]*sin(theta-pi/4) + L_wheel_distance*omega); //左上
			output = -1*(- V[X]/sin(theta+pi/4)+V[Y]/sin(theta-pi/4) + L_wheel_distance_front*V[ANGLE]*3/time)*1.3;
			break;
		case 4:
			//output =-1*( V[X]*sin(theta +3* pi / 4) - V[Y]*sin(theta + pi / 4) + L_wheel_distance*omega); //左下
			output =-1*( V[X]/sin(theta +3* pi / 4) - V[Y]/sin(theta + pi / 4) - L_wheel_distance*V[ANGLE]*3/time)*1.3;
			break;
		case 3:
			//output =-1*( V[X]*sin(theta + pi / 4) + V[Y]*sin(theta +3* pi / 4) + L_wheel_distance*omega); //右下
			output =-1*( V[X]/sin(theta + pi / 4) + V[Y]/sin(theta +3* pi / 4) + L_wheel_distance*V[ANGLE]*3/time)*1.3; 
			break;
		case 2:
...
void speed_control::robot_speed_B_set(void) {
	...
	V_B[ANGLE]= pid_yaw.PID_act(BEFORE_TARGET_POSI[ANGLE], my_posi->m_posi_angle_give());
